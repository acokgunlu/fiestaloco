...
import { CarState, freshCar, generateTrack, stepCar, MAX_SPEED } from '../../data/pistLogic';
...
export const KapismaControllerView: React.FC<Props> = ({
  ...
  /**
   * Pist yatay oranlı (1000x620). Dikey tutulan telefonda ekranın ancak
   * dörtte birini kaplıyor; yan çevirince 2,3 katı alan kaplıyor (ölçüldü:
   * 375x232'ye karşı 535x331). Zorlamıyoruz, hatırlatıyoruz.
   */
  const [dikey, setDikey] = useState(
    typeof window !== 'undefined' ? window.innerHeight > window.innerWidth : false,
  );
  useEffect(() => {
    const bak = () => setDikey(window.innerHeight > window.innerWidth);
    window.addEventListener('resize', bak);
    window.addEventListener('orientationchange', bak);
    return () => {
      window.removeEventListener('resize', bak);
      window.removeEventListener('orientationchange', bak);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const loop = (ts: number) => {
      ...
      if (car && !finishedRef.current) {
        const next = stepCar(car, steerRef.current, dt, track);
        carRef.current = next;
  ...
  // --- DİREKSİYON: parmağın BAŞLANGIÇ noktasına göre kayması.
  // Mutlak konum yerine göreli olması, başparmağı ekranın herhangi bir
  // yerine koyup sürebilmeyi sağlıyor — tek elle oynanabilirliğin şartı.
  const onDown = (clientX: number) => { touchOriginRef.current = clientX; steerRef.current = 0; };
  const onMove = (clientX: number) => {
    if (touchOriginRef.current === null) return;
    const dx = clientX - touchOriginRef.current;
    steerRef.current = Math.max(-1, Math.min(1, dx / FULL_LOCK_PX));
  };
  const onUp = () => { touchOriginRef.current = null; steerRef.current = 0; };

  // Masaüstünde ok tuşlarıyla da sürülebilsin — test ve klavyeli oynayanlar için.
  useEffect(() => {
    if (!racing) return;
    const bas = (e: KeyboardEvent) => {
      if (e.key === 'ArrowLeft') { steerRef.current = -1; e.preventDefault(); }
      if (e.key === 'ArrowRight') { steerRef.current = 1; e.preventDefault(); }
    };
    const birak = (e: KeyboardEvent) => {
      if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') steerRef.current = 0;
    };
      ...
      {gs.phase === 'LOBBY' && (
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 shadow-lg text-center space-y-3">
          ...
          <div className="p-3 rounded-2xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900 text-left text-xs space-y-1 font-medium text-rose-900 dark:text-rose-200">
            <p>{t('1️⃣ Parmağını ekrana koy ve sağa sola kaydır — direksiyon bu. Gaz otomatik.')}</p>
      ...
      */}
      {racing && createPortal(
        <div
          ...
          onPointerDown={(e) => { (e.target as Element).setPointerCapture?.(e.pointerId); onDown(e.clientX); }}
        ...
        >
          ...
          <div className="relative flex-1 min-h-0 flex items-center justify-center px-1">
            <PistCanvas
              seed={gs.seed}
              cars={gs.cars}
              players={players}
              ...
              className="mx-auto block rounded-xl shadow-2xl bg-[#2f6b3a] max-w-full max-h-full w-auto h-auto"
            />
            {finishedRef.current && (
          ...
          <p className="shrink-0 text-center text-[11px] font-black text-slate-500 py-1.5">
            {dikey
              ? t('Telefonu yan çevir — pist iki katı büyür.')
              : t('Parmağını kaydır — sağa sola. Sert kırarsan yavaşlarsın.')}
          </p>
        </div>,
        document.body,
      )}
